...
class App:
    ...
    def main(self):
        ...
        if self.selected_stock_name == None:
            col.button("Get Data", type="primary", use_container_width=True, disabled=True)  
        else:
            col.button("Get Data", on_click=self.downloadData, args = (start_data,), use_container_width=True, type="primary")
    ...
    def getMovingAvg(self):
        if not self.data.empty:
            ...
            with _lock:
                ...
                plt.plot(mov_avg_50, "Green", label="50 day moving avg", alpha=0.7)
                plt.plot(mov_avg_100, "Yellow", label="100 day moving avg", alpha=0.7)
                plt.plot(mov_avg_200, "Red", label="200 day moving avg", alpha=0.7)
                plt.plot(self.data["Close"], "Blue", label="Stock Closing Price", alpha=0.7)
